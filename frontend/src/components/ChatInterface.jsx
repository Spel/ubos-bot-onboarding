...
const ChatInterface = ({ 
  ...
  useEffect(() => {
    ...
    if (activeId && chats[activeId]) {
      ...
      // Handle agent selection based on props
      if (singleAgentMode && preSelectedAgentId) {
        // In single agent mode with preselected agent
        const agent = bots.find(bot => bot.id === preSelectedAgentId);
        if (agent) {
          setSelectedAgent(agent);
          setChatAgents([agent]);
        }
      } else if (preSelectedAgentId) {
        // Preselected agent but not in single agent mode
        const agent = bots.find(bot => bot.id === preSelectedAgentId);
        if (agent) {
          setSelectedAgent(agent);
          setChatAgents([agent]);
        ...
        // Use the chat's agent if available
        ...
        // Default to first agent if no active chat agent
        setSelectedAgent(bots[0]);
        setChatAgents([bots[0]]);
      }
  ...
  }, [singleAgentMode, preSelectedAgentId]);
  ...
  const createNewChat = () => {
    ...
    const newChat = {
      ...
      agentIds: selectedAgent ? [selectedAgent.id] : [],
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      messages: []
    };
  ...
  const switchToChat = (chatId) => {
    if (allChatSessions[chatId]) {
      ...
      // Set chat agents if available
      if (allChatSessions[chatId].agentIds && Array.isArray(allChatSessions[chatId].agentIds)) {
        const agents = allChatSessions[chatId].agentIds
          .map(id => availableAgents.find(bot => bot.id === id))
          .filter(agent => agent !== undefined);
        setChatAgents(agents);
      } else if (allChatSessions[chatId].agentId) {
        // For backward compatibility with single agent chats
        const chatAgent = availableAgents.find(bot => bot.id === allChatSessions[chatId].agentId);
        if (chatAgent) {
          setChatAgents([chatAgent]);
  ...
  const handleAgentChange = (e) => {
    // Only allow changing agents if not in single agent mode with preselected agent
    if (singleAgentMode && preSelectedAgentId) return;
    
    const agentId = e.target.value;
    const agent = availableAgents.find(agent => agent.id === agentId);
    setSelectedAgent(agent);
    
    // Update the current chat with the new agent if in single agent mode
    if (singleAgentMode && activeChatId && allChatSessions[activeChatId]) {
      const updatedChats = { ...allChatSessions };
      updatedChats[activeChatId] = {
        ...updatedChats[activeChatId],
        agentId: agent.id,
  ...
  const addAgentToChat = (agent) => {
    if (!agent || !activeChatId || singleAgentMode) return;
  ...
  const removeAgentFromChat = (agentId) => {
    if (!agentId || !activeChatId || singleAgentMode) return;
  ...
  const handleFileChange = (e) => {
    ...
    if (file && chatAgents.length > 0 && activeChatId) {
      // Add file message
      const fileMessage = {
        id: Date.now(),
      ...
      try {
        // Choose a random agent from the chat agents to respond
        const respondingAgent = chatAgents[Math.floor(Math.random() * chatAgents.length)];
        
        // Use the executeBot function from botStorage with file data
        const executionResult = executeBot(respondingAgent.id, { 
        ...
        addRecentActivity({
          type: 'file_upload',
          botId: respondingAgent.id,
          botName: respondingAgent.name,
          fileName: file.name,
          timestamp: new Date().toISOString()
        });
        ...
        setTimeout(() => {
          const errorResponse = {
            ...
            sender: "agent",
            agent: chatAgents[0], // Use the first agent for error messages
            timestamp: new Date().toISOString(),
            isError: true
          };
  ...
  const handleSendMessage = async (e) => {
    ...
    if (inputMessage.trim() === "" || chatAgents.length === 0 || !activeChatId) return;
    ...
    try {
      ...
      const executionResult = executeBot(respondingAgent.id, { input: inputMessage });
      ...
      setTimeout(() => {
        const botResponse = {
          id: Date.now() + 1,
          text: executionResult.result || `This is a simulated response from ${respondingAgent.name}.`,
          sender: "agent",
          agent: respondingAgent,
          timestamp: new Date().toISOString(),
        };
  ...
  return (
    <div className={`h-full ${darkMode ? 'bg-neutral-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      {/* Main Content - Using flex to ensure full height */}
      <div className="flex h-full">
        {/* Chat List Sidebar - Only show if not in single agent mode */}
        {!singleAgentMode && (
        ...
        {/* Chat Area */}
        <div className="flex-1 flex flex-col relative overflow-hidden" style={{ marginRight: isSidebarOpen ? '18rem' : '0', transition: 'margin-right 0.3s ease' }}>
          {/* Chat Header */}
          <div className={`p-4 border-b ${darkMode ? 'border-neutral-700' : 'border-gray-200'} flex justify-between items-center`}>
            ...
            <div className="flex items-center gap-4">
              {/* Toggle Agent Sidebar Button - Only show if not in single agent mode */}
              {!singleAgentMode && (
                <button 
                  ...
                  title={isSidebarOpen ? "Hide Agents Panel" : "Show Agents Panel"}
                >
              ...
              <div className="flex items-center">
                <div className={`py-1.5 px-3 text-sm font-medium rounded-md border ${
                  darkMode 
                  ...
                  {chatAgents.length === 0 ? (
                    ...
                    `${chatAgents[0].name} is active`
                  ) : (
                    `${chatAgents.length} agents active`
                  )}
                </div>
              </div>
            </div>
          ...
          <div className={`flex-1 overflow-y-auto p-4 ${darkMode ? 'bg-neutral-800' : 'bg-gray-50'}`} style={{ maxHeight: 'calc(100% - 130px)' }}>
            {messages.length === 0 ? (
          ...
          <div className={`p-4 border-t ${darkMode ? 'border-neutral-700' : 'border-gray-200'}`}>
            <form onSubmit={handleSendMessage} className="relative">
              ...
              <div className="absolute top-1/2 end-2 -translate-y-1/2 flex">
                ...
                <button 
                  type="submit"
                  disabled={chatAgents.length === 0 || inputMessage.trim() === ""}
                  className={`size-10 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-full border border-transparent ${
                    chatAgents.length === 0 || inputMessage.trim() === "" 
            ...
            {chatAgents.length === 0 && (
              <p className={`mt-2 text-sm ${darkMode ? 'text-red-400' : 'text-red-500'}`}>
                Please add at least one agent to start chatting
              </p>
            )}
          </div>
          
          {/* Right Sidebar for Managing Agents - Only show if not in single agent mode */}
          {isSidebarOpen && !singleAgentMode && (
            <div 
              className={`fixed right-0 top-0 bottom-0 w-72 border-l ${darkMode ? 'bg-neutral-800 border-neutral-700' : 'bg-white border-gray-200'} z-40 flex flex-col`}
              style={{ height: '100vh', transition: 'transform 0.3s ease', transform: isSidebarOpen ? 'translateX(0)' : 'translateX(100%)' }}
            >
              <div className="p-4 border-b border-gray-200 flex justify-between items-center">
                <h3 className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>Chat Agents</h3>
                <span className={`text-xs px-2 py-0.5 rounded-full ${darkMode ? 'bg-neutral-700 text-neutral-300' : 'bg-gray-100 text-gray-700'}`}>{chatAgents.length}</span>
              </div>
              ...
              <div className="p-4 overflow-y-auto" style={{ maxHeight: 'calc(100% - 120px)' }}>
                {chatAgents.length === 0 ? (
                  <div className={`text-center py-6 ${darkMode ? 'text-neutral-400' : 'text-gray-500'}`}>
                    ...
                    <p className="text-xl font-medium">No agents in this chat yet</p>
                    <p className="mt-2">Add an agent to start chatting</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {chatAgents.map(agent => (
                      <div 
                        key={agent.id} 
                        className={`p-3 rounded-lg border ${darkMode ? 'bg-neutral-700 border-neutral-600' : 'bg-white border-gray-200'}`}
                      >
          ...
          {/* Add Agent Modal */}
          {showAddAgentModal && !singleAgentMode && (
            <>
              {/* Overlay */}
              <div
                className="fixed inset-0 bg-gray-900/50 dark:bg-neutral-900/80 z-50"
                onClick={() => setShowAddAgentModal(false)}
                aria-label="Close modal overlay"
              />
              {/* Modal Content */}
              <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden pointer-events-none">
                <div className="mt-7 opacity-100 ease-out transition-all sm:max-w-4xl sm:w-full mx-4 pointer-events-auto">
                  <div className={`max-h-[90vh] overflow-hidden flex flex-col ${darkMode ? 'bg-neutral-900 border-neutral-800 shadow-neutral-700/70' : 'bg-white border-gray-200'} border shadow-2xl rounded-xl`}>
                    {/* Modal Header */}
