...
const AgentCreatorChat = ({ 
  ...
  return (
    <div 
      className={`fixed top-[61px] bottom-0 right-0 w-[30%] h-[calc(100vh-61px)] max-h-[calc(100vh-61px)] overflow-hidden z-30 flex flex-col ${darkMode ? 'bg-neutral-800 text-white' : 'bg-white text-gray-900'} shadow-xl border-l ${darkMode ? 'border-neutral-700' : 'border-gray-200'} transition-all duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
    >
      {/* Header */}
      ...
      <div className={`p-3 ${darkMode ? 'bg-neutral-700/50' : 'bg-gray-50'} border-b ${darkMode ? 'border-neutral-700' : 'border-gray-200'}`}>
        <div className="flex items-center gap-3">
          <div className="text-2xl bg-yellow-100 rounded-full w-8 h-8 flex items-center justify-center">{formData.avatar || '🤖'}</div>
          <div>
            <h4 className="font-medium">{formData.name || 'New Agent'}</h4>
            <p className="text-xs opacity-70">{formData.type || 'sales'}</p>
          </div>
        </div>
      </div>
      ...
      {/* Input */}
      <div className={`p-3 border-t ${darkMode ? 'border-neutral-700' : 'border-gray-200'}`}>
        <div className="flex items-center gap-2 bg-white dark:bg-neutral-700 rounded-lg border border-gray-300 dark:border-neutral-600 pr-1 overflow-hidden">
          <input
            type="text"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()}
            placeholder="Ask for help or describe what you want to create..."
